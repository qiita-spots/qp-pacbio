...
class PacAdapterRmTests(PacBioTests):
    def test_pacbio_adapter_removal(self):
        ...
        exp_main = [
            ...
            "filename=`echo $input | awk '{print $2}'`\n",
            "fn=`basename ${filename}`\n",
            "fout={out_folder}/lima/${fn}\n",
            "final={out_folder}/final/${fn}\n",
            "\n",
            f'lima "${{filename}}" {out_dir}/adapter.fasta "${{fout}}.fastq.gz" --hifi-preset SYMMETRIC --peek-guess --min-score 0 --min-end-score 0 --min-ref-span 0 --min-scoring-regions 0 --min-score-lead 0 --min-length 0\n',
            "\n",
